...
export default function WithoutSimbian() {
  ...
  const [ignoredAlertsList, setIgnoredAlertsList] = useState<
    Array<{ id: string; text: string; time: string }>
  >([]);
  const [wronglyClosedList, setWronglyClosedList] = useState<
    Array<{ id: string; text: string; time: string }>
  >([]);
  ...
  return (
    <div className="space-y-8">
      <h2 className="text-center text-2xl font-bold text-red-600 md:text-3xl">
        Without Simbian
      </h2>
      ...
      <div className="mt-8 rounded-lg bg-red-50 p-4 text-center">
        <p className="text-lg font-medium text-red-800">
          Security teams are overwhelmed with alerts, missing critical threats,
          and wasting time on false positives
        </p>
      </div>
    </div>
  );
...
function AlertCard({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`rounded border-2 p-4 flex flex-col h-full ${borderColorMap[color]} ${bgColorMap[color]}`}
        >
          ...
          <div className="flex flex-col flex-grow">
            ...
            <div
              className={`space-y-2 overflow-y-auto overflow-x-hidden pr-1 scrollbar-thin ${bgColorMap[color]} flex-grow`}
            >
              <AnimatePresence>
                {alerts.map((alert) => (
